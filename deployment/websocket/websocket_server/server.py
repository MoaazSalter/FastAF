...
# FastAPI app
app = FastAPI(title="FastAF Drug Detection API")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_methods=["*"],
    ...
    try:
        while True:
            ...
            try:
                ...
                # Run inference
                try:
                    outputs = await asyncio.to_thread(ort_session.run, None, {"images": input_tensor})
                except Exception as e:
                ...
                response = DetectionResponse(
                    request_id=request_id,
                    timestamp=datetime.now(timezone.utc).timestamp(),
                    processing_time_ms=(asyncio.get_running_loop().time() - start_time) * 1000,
                    detections=boxes
                )
                await websocket.send_json(response.model_dump())
